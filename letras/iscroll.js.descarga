...
iScroll.prototype = {
	...
	_start: function (e) {
		...
		//MODIFICADO
		//La primera vez que hacemos zoom o click con iscroll actualizamos la escala actual
		if(controlZoom == 1) {this.scale = escala; controlZoom = 0;}
	...
	_move: function (e) {
		...
		if (that.options.zoom && hasTouch && e.touches.length > 1) {
			...
			//MODIFICADO
			//Asignamos a nuestra escala la escala aplicada por iScroll para mantenerla actualizada y con los márgenes adaptados correctamente
	...
	_end: function (e) {
		...
		if (that.zoomed) {
			...
			//MODIFICADO
			//Actualizamos la posicion en la que estamos para que si hacemos zoom desde los botones lo haga desde ahí
			actualX = that.x;
			actualY = that.y;
